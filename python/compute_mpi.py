...
solution_file = path_to_results /f'Solution_nx{Nx}_{MapSize}km_T{Tend}_nprocess{size}_h.bin'
solution_txt = path_to_results /f'Solution_nx{Nx}_{MapSize}km_T{Tend}_nprocess{size}_h.txt'
...
while Tn < Tend:
    ...
    # Report Status
    print(f'{n_steps:04d} - Computing T: {Tn + dT} ({100 * (Tn + dT) / Tend}%) - dT: {dT} hr ') if (rank==0) and (n_steps%10==0) else None
    ...
    i = [slice(1, local_size[0]-1), slice(2, local_size[0]), slice(0, local_size[0]-2)]
    j = [slice(1, local_size[1]-1), slice(2, local_size[1]), slice(0, local_size[1]-2)]
...
cartesian2d.Gatherv(np.ascontiguousarray(h[i[0], j[0]]), H, root=0)
